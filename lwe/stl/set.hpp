...
public:
    const Iterator find(const T&) const; //!< find by data
    const Iterator find(hash_t) const;   //!< find by hash
    const Iterator at(size_t) const;     //!< find by order
    const Iterator begin() const;        //!< get begin
    const Iterator end() const;          //!< get end

public:
    Iterator find(const T&); //!< non-const
    Iterator find(hash_t);   //!< non-const
    Iterator at(size_t);     //!< non-const
...
template<typename T> auto Set<T>::find(const T& in) const -> const Iterator {
    hash_t  hashed = util::hashof<T>(in);      // get hash
    index_t index  = hashed & (capacitor - 1); // get index
...
template<typename T> auto Set<T>::find(hash_t in) const -> const Iterator {
...
template<typename T> auto Set<T>::at(size_t index) const -> const Iterator {
    size_t pass = 0;
    // out of range
    if(index >= counter) {
        return end();
...
template<typename T> auto Set<T>::begin() const -> const Iterator {
    size_t index = 0;
...
template<typename T> auto Set<T>::end() const -> const Iterator {
    return Iterator(this, capacitor);
}
